...
public class DaoConversa {
	...
	public static void guardaMensagem(Mensagem mensagem, Conversa conversa) {
		...
		try {
			st = conn.prepareStatement(
					"INSERT INTO mensagens(destinatario, remetente, data, mensagem) VALUES (?, ?, ?, ?)");
			st.setInt(1, mensagem.getDestinatario().getId());
			st.setInt(2, mensagem.getRemetente().getId());
			st.setTimestamp(3,
					new java.sql.Timestamp(ConversorDataUtils.getDateTimeToDate(mensagem.getDateTime()).getTime()));
	...
	public static void guardaMensagemComArquivo(Mensagem mensagem, Conversa conversa, String caminhoArquivoServidor,
			String caminhoArquivoRemetente) {
		PreparedStatement st = null;
		Connection conn = DbUtils.getConnection();
		guardaUltimaMensagem(mensagem, conversa, caminhoArquivoServidor, caminhoArquivoRemetente);
		try {
			st = conn.prepareStatement(
					"INSERT INTO mensagens(destinatario, remetente, data, mensagem, enderecoArquivoServidor, enderecoArquivoRemetente) VALUES (?, ?, ?, ?, ?, ?)");
			st.setInt(1, mensagem.getDestinatario().getId());
			st.setInt(2, mensagem.getRemetente().getId());
			st.setTimestamp(3,
					new java.sql.Timestamp(ConversorDataUtils.getDateTimeToDate(mensagem.getDateTime()).getTime()));
	...
	public static void guardaUltimaMensagem(Mensagem mensagem, Conversa conversa, String caminhoArquivoServidor,
			String caminhoArquivoRemetente) {
		PreparedStatement st = null;
		Connection conn = DbUtils.getConnection();
		if (caminhoArquivoServidor == null) {
			try {
				if (conversa == null) {
					st = conn.prepareStatement(
							"REPLACE INTO conversas(usuario1, usuario2, ultimaMensagem, data) VALUES (?, ?, ?, ?)");
					st.setInt(1, mensagem.getDestinatario().getId());
					st.setInt(2, mensagem.getRemetente().getId());
					st.setString(3, mensagem.getMensagem());
					st.setTimestamp(4, new java.sql.Timestamp(
							ConversorDataUtils.getDateTimeToDate(mensagem.getDateTime()).getTime()));
				} else {
					st = conn.prepareStatement(
							"REPLACE INTO conversas(id, usuario1, usuario2, ultimaMensagem, data) VALUES (?, ?, ?, ?, ?)");
			...
			try {
				if (conversa == null) {
					st = conn.prepareStatement(
							"REPLACE INTO conversas(usuario1, usuario2, ultimaMensagem, data, enderecoArquivoServidor, enderecoArquivoRemetente) VALUES (?, ?, ?, ?, ?, ?)");
					st.setInt(1, mensagem.getDestinatario().getId());
					st.setInt(2, mensagem.getRemetente().getId());
					st.setString(3, mensagem.getMensagem());
					st.setTimestamp(4, new java.sql.Timestamp(
					...
					st = conn.prepareStatement(
							"REPLACE INTO conversas(id, usuario1, usuario2, ultimaMensagem, data, enderecoArquivoServidor, enderecoArquivoRemetente) VALUES (?, ?, ?, ?, ?, ?, ?)");
					st.setInt(1, conversa.getId());
					st.setInt(2, mensagem.getDestinatario().getId());
					st.setInt(3, mensagem.getRemetente().getId());
					st.setString(4, mensagem.getMensagem());
	...
	public static Vector<Object> buscaHistorico(Vector<Object> request) throws IOException {
		...
		try {
			...
			st = conn.prepareStatement(
					"select * from mensagens where (destinatario = ? and remetente = ?) or (destinatario = ? and remetente = ?);");
			...
			while (rs.next()) {
				...
				if (rs.getString("enderecoArquivoServidor") != null) {
					arquivo = new Arquivo(null, new File(rs.getString("enderecoArquivoRemetente")),
							new File(rs.getString("enderecoArquivoDestinatario")),
							new File(rs.getString("enderecoArquivoServidor")));
				}
				if (remetente.getId() != rs.getInt("remetente")) {
					conversa.add(new Mensagem(rs.getString("mensagem"), destinatario, remetente,
							rs.getTimestamp("data").toLocalDateTime(), arquivo));
				} else {
					conversa.add(new Mensagem(rs.getString("mensagem"), remetente, destinatario,
							rs.getTimestamp("data").toLocalDateTime(), arquivo));
				}
			}
	...
	public static Vector<Object> buscaConversas(Vector<Object> request) throws IOException {
		...
		try {
			...
			st = conn.prepareStatement("select * from conversas where (usuario1 = ? or usuario2 = ?);");
			...
			while (rs.next()) {
				...
				if (rs.getString("enderecoArquivoServidor") == null) {
					conversas.add(new Conversa(rs.getInt("id"), new Mensagem(rs.getString("ultimaMensagem"),
							requisitante, temp, rs.getTimestamp("data").toLocalDateTime()), requisitante, temp));
				} else {
					...
					conversas.add(new Conversa(rs.getInt("id"),
							new Mensagem(rs.getString("ultimaMensagem"), requisitante, temp,
									rs.getTimestamp("data").toLocalDateTime(),
									new Arquivo(null, arquivoNoRemetente, null, arquivoNoServidor)),
							requisitante, temp));
				}
			}
